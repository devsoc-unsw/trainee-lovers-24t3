...
function initializeSocketServer(server) {
  ...
  ioInstance.on("connection", (socket) => {
    ...
    socket.on("add-question", (roomCode, questionContent, callback) => {
      try {
        addQuestion(questionContent, roomCode);
      } catch (error) {
        console.error(error);
    ...
    socket.on("player-loaded-round", (roomCode, userId) => {
      if (rooms[roomCode]) {
        ...
        const allUsersLoaded = rooms[roomCode].users.every(
          (user) => user.roundLoaded === true
        );
        let intervalId = null;
        if (allUsersLoaded && rooms[roomCode].gameStart) {
          ...
          intervalId = setInterval(() => {
            if (rooms[roomCode] && rooms[roomCode].timer > 0) {
              ...
              ioInstance
                .to(roomCode)
                .emit("countdown-update", rooms[roomCode].timer);
            } else {
    ...
    socket.on("get-word", (roomCode, categoryName) => {
      ...
      const allUsersLoaded = room.users.every(
        (user) => user.roundLoaded === true
      );
      if (allUsersLoaded) {
    ...
    socket.on("generate-identity", (roomCode, numCyborgs) => {
      ...
      if (allUsersLoaded) {
        ...
        for (let i = 0; i < users.length; i++) {
          ioInstance
            .to(users[i].socket)
            .emit("identity-generated", identities[i]);
        }
      } else {
